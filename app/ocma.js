    ...
    if ((t && typeof t == 'object') || typeof t == 'function')
      for (let s of Tt(t))
        !Q.call(r, s) &&
          ...
          w(r, s, {
            get: () => t[s],
            enumerable: !(e = Rt(t, s)) || e.enumerable,
          });
    return r;
  },
  g = r =>
...
var nt = Lt(_ => {
    ...
    D = (function () {
      ...
      return (
        (r._xfnv1a = function (t) {
          for (var e = 2166136261, s = 0; s < t.length; s++)
  ...
  (_.PRNG = void 0),
    ((C = _.PRNG || (_.PRNG = {})).sfc32 = 'sfc32'),
    (C.mulberry32 = 'mulberry32'),
    (C.xoshiro128ss = 'xoshiro128ss');
  var jt = (function () {
    function r(t, e) {
      e === void 0 && (e = _.PRNG.sfc32),
        (this.str = t),
        (this.prng = e),
...
var B = g(require('path')),
  tt = [
    {
      flag: '-d',
      desc: 'Indica que as sa\xEDdas de debug devem ser impressas.',
      process: r => (r.debug = !0),
      num: 0,
    },
    {
    ...
    {
      flag: '-i',
      desc: 'Entra no modo iterativo. A cada rodada, o jogo espera o usu\xE1rio digitar <enter> para continuar.',
      process: r => (r.interactive = !0),
      num: 0,
    },
    {
      flag: '-m',
      desc: 'Salva as mensagens enviadas para os bots, \xFAtil para debugar. Cada bot ter\xE1 seu arquivo com a extens\xE3o .in',
      process: r => (r.messages = !0),
      num: 0,
    },
    {
      flag: '-r',
      desc: 'L\xEA o estado do jogo especificado em um arquivo, sobrescrevendo argumentos passados. Exemplo: -r game.state',
      process: (r, t, e) => (r.readfile = t[e + 1]),
      num: 1,
    },
    {
      flag: '-s',
      desc: 'Indica uma semente para o gerador de n\xFAmero aleat\xF3rios. A semente deve ser um valor inteiro. Exemplo: -s 24652',
      process: (r, t, e) => (r.seed = parseInt(t[e + 1])),
      num: 1,
    },
    {
      flag: '-t',
      desc: 'Indica o tempo (em milisegundos) que um bot tem para responder com uma a\xE7\xE3o antes de dar timeout (default: 3000). Exemplo: -t 2000',
      process: (r, t, e) => (r.timeout = parseInt(t[e + 1])),
      num: 1,
    },
  ];
function Bt(r, t = []) {
...
function et(r = '', t = []) {
  ...
  return (
    tt.concat(t).forEach(o => {
      ...
      a > 1 &&
        a < e.length - o.num &&
        (o.process(s, e, a), e.splice(a, o.num + 1));
    }),
...
var n = Object.keys(R).reduce(
  (r, t) => Z(P({}, r), { [t]: e => Ft(`${R[t]}${e}`) }),
  {}
);
var d = console.log,
  ...
  $ = class {
    constructor(t, e, s) {
      ...
      i(this, '_active');
      I.dirname(s) === '.'
        ? (this._proc = (0, N.spawn)(I.join(process.cwd(), s)))
        : (this._proc = (0, N.spawn)(s)),
        this.game.config.messages &&
          (this._msgStream = (0, ot.createWriteStream)(s + '.in')),
        this.game.config.quiet && (d = () => {}),
        this._proc.on('error', o => {
          d(n.red(`Erro na execu\xE7\xE3o do programa ${s}.`)),
            d(n.red(o)),
            this.game.quit(e, o);
        }),
	...
	`
        ...
        (this._errorPromise = new Promise(o => {
          this._proc.on('close', (a, u) => {
            u !== 'SIGINT'
              ? (d(
                  n.red(`Programa ${this.execName} terminou com signal ${u}.`)
                ),
                o('EXEC_ERROR'))
              : o('FINISHED');
          });
    ...
    async write(t) {
      if (this._active) {
        this.game.config.debug &&
          console.debug(`${it} Enviando para ${this.id}: ${t}`);
        let e = `${t}
`;
    ...
    async read() {
      if (this._active) {
        let t,
          ...
          a = await Promise.race([e, s, o]).then(u =>
            u === void 0 || u === 'EXEC_ERROR' ? 'ERROR' : u
          );
        return a === 'TIMEOUT'
          ? (d(
              `Jogador ${this.id} n\xE3o enviou dados no tempo de ${this.game.config.timeout}ms e foi eliminado.`
            ),
            Promise.resolve('PLAYER_ERROR'))
          : a === 'ERROR'
          ? (d(
              `Jogador ${this.id} teve problemas t\xE9cnicos e foi eliminado.`
            ),
            Promise.resolve('PLAYER_ERROR'))
          : (clearTimeout(t),
            this.game.config.debug &&
              console.debug(`${it} Recebido de ${this.id}: ${a}`),
            Promise.resolve(a));
      }
    }
    halt() {
      console.log(n.red('HALT')),
        this._msgStream && this._msgStream.close(),
...
function v(r) {
  return (
    typeof r == 'number' && ((j = new at.default(r.toString())), (ct = r)), ct
  );
}
function y() {
...
var T = console.log,
  ...
  O = class {
    constructor(t) {
      ...
      (this.config = P(P({}, t), e)),
        ...
        (!this.currPlayerIndex ||
          ...
          this.currPlayerIndex > this.config.bots.length - 1) &&
          (this.currPlayerIndex = h(this.config.bots.length)),
        (this.players = this.config.bots.map(
          s => new $(this, ut.basename(s), s)
        )),
        this.players.forEach((s, o) => (s.color = Vt[o]));
    }
    async run() {
      try {
        for (await this.config.init(), await this.config.start(); ; )
          await this.config.turn(),
            (this.currPlayerIndex =
              (this.currPlayerIndex + 1) % this.players.length);
      } catch (t) {
    ...
    quit(t, e) {}
    closeGame() {
      console.log(n.red('closeGame')),
        this.players.forEach(t => t.halt()),
        process.exit();
    }
    loadFile(t) {
      try {
...
`),
        ...
        u &&
          (this.config.bots = u[1].split(' ').filter(J => J && J.length > 0));
    ...
    async send(t, e) {
      ...
      if (s) {
        ...
        return o !== 'OK'
          ? (this.failPlayer(
              'CONFIG ERROR',
              `Algo estranho ocorreu com o jogador ${s.id}`,
              s
            ),
            'ERROR')
          : o;
...
var L = class {
  ...
  E = class {
    constructor() {
      ...
      let t = () =>
        y() < 0.5 ? G + h(10, 1) : y() < 0.7 ? H + h(10, 1) : M + h(10, 1);
      this._cell = [];
      for (let e = 0; e < this.height; e++) {
        this._cell.push([]);
        for (let s = 0; s < this.width; s++)
          this._cell[e].push(y() <= 0.3 ? t() : 0);
      }
      this.harbors = [];
...
var W = class {
  ...
  unload() {
    let t =
      this.quantity('MULLET') * At +
      this.quantity('SNAPPER') * _t +
      this.quantity('SEABASS') * Pt;
    ...
    move(t) {
      ...
      return (
        ...
        e.isInside(s)
          ? b.isBusy(s)
            ? new c('BUSY')
            : (this._boat.moveTo(s), new c('DONE'))
          : ((this._money -= dt), new c('OUT'))
      );
    }
    fish() {
      let t = p.instance,
      ...
      return e
        ? t.isAllowedToFish(this.pos)
          ? (t.fish(this.pos), this._boat.load(e.fish), new c(e.fish))
          : ((this._money -= yt), new c('IMPACT'))
        : new c('NONE');
    }
    sell() {
...
var x = 500,
  ...
  Ht = [
    {
      ...
      process: (r, t, e) => {
        (x = parseInt(t[e + 1])),
          console.log('M\xE1ximo de rodadas alterado para: ', x);
      },
      num: 1,
    },
    {
      ...
      process: (r, t, e) => {
        (z = parseInt(t[e + 1])),
          console.log('Saldo vencedor alterado para: ', z);
      },
      num: 1,
    },
  ],
  m = new O({
    description:
      'Neste jogo, voc\xEA controla um barco-pesqueiro, que compete com outros barcos pelos recursos naturais de uma \xE1rea de pesca.',
    loadState: Wt,
    init: kt,
    start: Yt,
    turn: zt,
...
async function kt() {
  m.numPlayers === 0 &&
    (console.log('N\xE3o h\xE1 bot-jogadores.'), m.closeGame()),
    (xt = 0),
    m.forEachPlayer(r => new f(r.id));
}
async function Yt() {
  let r = p.instance;
...
async function zt() {
  ...
  try {
    if (
      (++xt % m.numPlayers == 0 && (console.log(`Faltam ${x} rodadas.`), x--),
      x >= 0)
    ) {
      let t = m.currentPlayer.id;
      await m.send(t, new c(r.strData)),
        await m.send(t, new c('BOTS', f.bots.length));
      for (let a of f.bots) await m.send(t, new c(a.id, a.pos.lin, a.pos.col));
      let e = await m.receiveMessage(),
        s = f.bot(t),
        o = s.valid(e);
      await m.send(t, o),
        s.money >= z && m.over('Um jogador alcan\xE7ou o valor m\xE1ximo!'),
        r.update(),
        Y();
    } else m.over('timeout');
  } catch (t) {
...
function Jt() {
  console.log('COLOCA\xC7\xC3O'),
    f.bots
      .sort((r, t) => t.money - r.money)
      .forEach((r, t) =>
        console.log(`${t + 1}\xBA: ${r.id} - R$ ${r.money},00`)
      );
}
